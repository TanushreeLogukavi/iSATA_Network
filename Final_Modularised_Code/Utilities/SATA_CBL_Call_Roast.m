...
SATA_CBL_check_init;
global ROAST_PATH SATA_PATH
% assign default head model
if nargin<=7
    head_model = [ROAST_PATH filesep 'example' filesep 'MNI152_T1_1mm.nii'];
end
...
folder = [electrode_loc1 '_' electrode_loc2 '_' num2str(electrode_size(1)) '_' num2str(electrode_size(2))];

% find current working directory
mydir  = pwd;
idcs   = strfind(mydir,filesep);
newdir = mydir(1:idcs(end));
working_dir = [ROAST_PATH filesep 'example'];
...
roast(head_model,{electrode_loc1,anode_current,electrode_loc2,cathode_current},'electype',electrode_type...
    ,'elecsize',electrode_size, 'captype',cap_type,'resampling','on');
cd(rever);
...
mkdir(save_dir,folder);
save_dir = [save_dir filesep folder];
...
for x =1:l
    ...
    if ~any(ismember(names,name_cur))
        movefile([working_dir filesep name_cur],save_dir);
    end
end
