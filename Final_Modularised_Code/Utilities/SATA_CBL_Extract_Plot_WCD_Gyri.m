...
if brain_parts == 1
    ...
    center_of_mass_x = zeros(2*l,1);
    center_of_mass_y = zeros(2*l,1);
    center_of_mass_z = zeros(2*l,1);
    ...
    for x=1:l
        ...
        if isempty(records1)
            ...
            center_of_mass_x(2*x-deled-1) = [];
            center_of_mass_y(2*x-deled-1) = [];
            center_of_mass_z(2*x-deled-1) = [];
            
            deled = deled +1;
        else
            temp1 = CD_array(records1);
            Gyrus{2*x-deled-1}=char("Left "+lev3_values(x));
            weighted(2*x-deled-1)=mean(temp1);%length(temp1)*
            
            center_of_mass_x(2*x-deled-1) = mean(tal_in.ori_x(records1));
            center_of_mass_y(2*x-deled-1) = mean(tal_in.ori_y(records1));
            center_of_mass_z(2*x-deled-1) = mean(tal_in.ori_z(records1));
        ...
        if isempty(records2)
            ...
            center_of_mass_x(2*x-deled) = [];
            center_of_mass_y(2*x-deled) = [];
            center_of_mass_z(2*x-deled) = [];
            
            deled=deled+1;
        else
            temp2 = CD_array(records2);
            Gyrus{2*x-deled}=char("Right "+lev3_values(x));
            weighted(2*x-deled)=mean(temp2); %length(temp2)*
            
            center_of_mass_x(2*x-deled) = mean(tal_in.ori_x(records2));
            center_of_mass_y(2*x-deled) = mean(tal_in.ori_y(records2));
            center_of_mass_z(2*x-deled) = mean(tal_in.ori_z(records2));
...
% form the table
weighted_table = table(Gyrus,weighted,center_of_mass_x,center_of_mass_y,center_of_mass_z);
